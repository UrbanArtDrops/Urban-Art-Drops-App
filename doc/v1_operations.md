...
- Local self-service registration now starts a tokenized email-verification flow: the account is not login-ready until the user opens the `/auth/verify-email?userId=...&token=...` link generated by `POST /api/auth/register-local`; tokens are hashed at rest and expire after 24 hours
- Users can request another verification link through `POST /api/auth/email-verification/request`, which returns a generic response to avoid account enumeration
- Local password reset uses `POST /api/auth/password-reset/request` and `POST /api/auth/password-reset/complete`; reset tokens are hashed at rest, expire after 1 hour, and successful reset clears login backoff state
- Verification and reset links use `PublicAppBaseUrl` from admin configuration, or the current API request origin as a fallback, and require working SMTP settings for real mail delivery
